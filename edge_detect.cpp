...
int main(int argc, char *argv[])
{
    ...
    for (j = 0; j < Y_dim; j++){
        ...
        for (i = 0; i < X_dim; i++){
            ...
            if(n_elements % 2 == 0)
                outputImg[0][j][i] = (distfrac[(n_elements-1)/2] + distfrac[n_elements/2])/2.0;
            else
                outputImg[0][j][i] = distfrac[n_elements/2];
    ...
    for (j = 0; j < Y_dim; j++)
    {
            poBand->RasterIO( GF_Write, 0, j, X_dim, 1, outputImg[0][j], X_dim, 1, GDT_Float64, 0, 0 );
    }
